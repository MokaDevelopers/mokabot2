...
inner_pkgs = ['nonebot_plugin_manager', 'mb2pkg_admin', 'nonebot_plugin_statistical']

npm = on_shell_command("npm", parser=npm_parser, priority=1)


# 在 Matcher 运行前检测其是否启用
async def check_plugin_permission(matcher: Matcher, bot: Bot, event: MessageEvent, state: T_State):

    plugin_manager = PluginManager()
    plugin = matcher.module.split(".", maxsplit=1)[0]

    conv = {
        "user": [event.user_id] if isinstance(event, MessageEvent) else [],
        "group": [event.group_id] if isinstance(event, GroupMessageEvent) else [],
    }

    plugin_manager.update_plugin(
        {
            str(p.name): p.name not in inner_pkgs and p.matcher
