...
async def get_random_song(message: str) -> tuple[BytesIO, str]:
    ...
    cover = Image.open(IGRMngr.get_song_cover(chart.id, chart.song_info.remote_download, chart.song_info.jacket_override))
    return (
        save_to_bytesio(cover),
        generate_song_info_text([chart.song_info], difficulty_index=chart.ratingClass)
    )
