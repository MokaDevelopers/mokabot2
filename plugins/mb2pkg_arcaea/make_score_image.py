...
class Fonts:
    ...
    exo_16_2 = ImageFont.truetype(os.path.join(ARCLASTDIR, 'fonts', 'Exo-Medium.ttf'), 16 + 4)
...
def andreal_v2_draw_recent(data: UniversalProberResult):
    ...
    slogan = 'Generated by Lagrange, powered by project Andreal'
    draw.text((10, 1080 - draw.textsize(text=slogan, font=Fonts.exo_16_2)[1] - 10), slogan, font=Fonts.exo_16_2, stroke_width=1, stroke_fill=(82, 82, 82))
...
def andreal_v3_draw_recent(data: UniversalProberResult):
    ...
    slogan = 'Generated by Lagrange, powered by project Andreal'
    draw.text(((1000 - draw.textsize(text=slogan, font=Fonts.exo_16_2)[0]) / 2, 1444 - draw.textsize(text=slogan, font=Fonts.exo_16_2)[1] - 10), slogan, font=Fonts.exo_16_2, fill=(110, 110, 110))
...
async def andreal_draw_b30(data: UniversalProberResult):
    img = Image.open(os.path.join(ARCLASTDIR, 'res_andreal', 'B30.png'))
    account_info = data.user_info
    color_list = [(20, 165, 215), (120, 155, 80), (115, 35, 100), (165, 20, 49)]
    draw = ImageDraw.Draw(img)
    ptt = account_info.rating / 100
    best30_avg = sum([r.rating for r in data.scores][:30]) / 30
    recent10_avg = 4 * ptt - 3 * best30_avg
    draw.text((345, 175), f'{account_info.name}', (255, 255, 255), font=Fonts.andrea_72_2)
    draw.text((370, 320), f'ArcID: {account_info.code}', (255, 255, 255), font=Fonts.exolight_28)
    draw.text((145, 480), f'Total Best 30: {round(best30_avg, 4)}', (255, 255, 255), font=Fonts.andrea_60_2)
    draw.text((1095, 480), f'Recent Best 10: {round(recent10_avg, 4)}', (255, 255, 255), font=Fonts.andrea_60_2)
    slogan = 'Generated by Lagrange, powered by project Andreal'
    draw.text((1000, 5900), slogan, (255, 255, 255), font=Fonts.kazasawa_regulary_30_2, stroke_width=3,
              stroke_fill=(82, 82, 82))
    partner = get_partner_icon(account_info.character, account_info.is_char_uncapped)
    img.paste(partner, (75, 130), partner)  # To add the character icon from function above
    rating = get_rating_image(account_info.rating)
    img.paste(rating, (175, 235), rating)  # To add the rating data from function above
    i = 0

    for records in data.scores[:30]:
        x = 75 + (0 if i % 2 == 0 else 950)
        y = 660 + i // 2 * 350
        position_list_f = [(x + 9, y), (x + 891, y), (x + 900, y + 9), (x + 900, y + 291),
                           (x + 891, y + 300), (x + 9, y + 300), (x, y + 291), (x, y + 9)]
        position_list_difficulty = [(x + 278, y + 22), (x + 278, y + 70), (x + 503, y + 70), (x + 458, y + 22)]
        draw.polygon(position_list_f, fill=(255, 255, 255))
        songname = songtitle[records.song_id]["en"]
        if len(songname) > 15:  # To restrict the length of the songname
            final_songname = songname[0:15] + '...'
        else:
            final_songname = songname
        draw.polygon(position_list_difficulty, fill=color_list[records.difficulty])  # To draw the polygon of difficulty

        img.paste(get_song_image(records.song_id, records.difficulty)[0].resize((256, 256)), (x + 22, y + 22))
        sys_color = get_song_image(records.song_id, records.difficulty)[1]

        str_score = str(records.score).zfill(8)
        formatted_score = f"{str_score[-8:-6]}'{str_score[-6:-3]}'{str_score[-3:]}"
        draw.text((x + 295, y + 80), f'[{calc_last_const(records.score, records.rating)}] '
                                     f'{final_songname}', sys_color, font=Fonts.beatrice_36)
        draw.text((x + 290, y + 145), formatted_score, sys_color, font=Fonts.exo_44_2)
        draw.text((x + 297, y + 24), "%.4f" % records.rating, (255, 255, 255), font=Fonts.exo_26_2)
        draw.text((x + 800, y + 24), f'#{i + 1}', (0, 0, 0), font=Fonts.beatrice_26_2)
        draw.text((x + 300, y + 235), f'Pure: {records.perfect_count} (+{records.shiny_perfect_count})',
                  (105, 68, 100), font=Fonts.beatrice_20)
        draw.text((x + 570, y + 235), f'Far: {records.near_count}', (216, 157, 49), font=Fonts.beatrice_20)
        draw.text((x + 730, y + 235), f'Far: {records.miss_count}', (159, 83, 109), font=Fonts.beatrice_20)
        i += 1

    # return
    make_path = os.path.join(temp_absdir, f'{account_info.code}_andreal_best30.png')
    img.save(make_path)
    return make_path
